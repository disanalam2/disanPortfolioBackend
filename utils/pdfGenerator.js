...
async function generateAuditPDF(businessName, websiteIssues) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // The Issues Box
            doc.rect(50, doc.y, 500, 150).fillAndStroke('#fee2e2', '#ef4444');
            doc.fillColor('#991b1b').text(' IDENTIFIED ISSUES:', 60, doc.y + 15, { bold: true });
            doc.moveDown(0.5);
            doc.fillColor('#7f1d1d').text(websiteIssues || "Slow mobile loading speed, poor SEO structure, and lack of clear Call-To-Action buttons.", {
                width: 480,
                align: 'left'
            });
